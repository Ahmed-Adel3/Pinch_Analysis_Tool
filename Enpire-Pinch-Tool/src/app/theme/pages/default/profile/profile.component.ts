...
import { profileData, CaseData } from './classes';
...
export class ProfileComponent implements OnInit {
    ...
    resetPassErrors: string = '';
    resetPassSuccess: string = '';

    casesData;

    constructor(private _pData: ProfileDataService,
        ...
        this._pData.getProfileData().subscribe(data => {
    ...
    SetInitialValues() {
        this.EditProfileForm.get('firstName').setValue(this.Data.firstName);
        this.EditProfileForm.get('lastName').setValue(this.Data.lastName);
        this.EditProfileForm.get('userName').setValue(this.Data.userName);
        this.EditProfileForm.get('email').setValue(this.Data.email);
        this.EditProfileForm.get('country').setValue(this.Data.country);
        this.EditProfileForm.get('company').setValue(this.Data.company);
        this.EditProfileForm.get('position').setValue(this.Data.position);

        this.CountriesWithoutSelected = this.Countries.filter(a => a.name !== this.Data.country)
    }

    EditProfile(e) {
        this._pData.setProfileData(e).subscribe(data => {
    ...
    resetPassword(e) {
        ...
        this._pData.resetPassword(e).subscribe(data => {
            this.resetPassErrors = '';
            this.resetPassSuccess = data;
            setInterval(() => { this.resetPassSuccess = '' }, 4000)
        },
            error => {
                ...
                for (let i = 0; i < fieldsErrorsArr.length; i++) {
                    for (let j = 0; j < fieldsErrorsArr[i].length; j++) {
                        this.resetPassErrors += " , " + fieldsErrorsArr[i][j]
                    }
                }
                this.resetPassErrors = this.resetPassErrors.substring(2, this.resetPassErrors.length - 1)
            })
    }

    getcases() {
        this._pData.getCases().subscribe(
            data => {
    ...
    deleteCase(e, id) {
        this._pData.deleteCase(id).subscribe(data => { },
            error => { console.log(error) });
        this.casesData = this.casesData.filter(a => a.Id !== id)
    }
